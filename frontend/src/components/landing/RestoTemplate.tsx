    ...
    <div className="w-full">
      ...
      {featuresSec.show && config.features && config.features.length > 0 && (
        <section className="py-16 px-6 max-w-6xl mx-auto border-t border-[var(--border)]">
          <div className="text-center mb-12">
            <h2 className="text-xl font-extrabold font-serif">{featuresSec.title}</h2>
          </div>
      ...
      {howWeServeSec.show && config.howWeServe && config.howWeServe.length > 0 && (
        <section className="py-20 px-6 max-w-5xl mx-auto border-t border-[var(--border)]">
          <div className="text-center mb-16">
            <h2 className="text-xl font-extrabold font-serif mb-4">{howWeServeSec.title}</h2>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-5 relative">
            ...
            {config.howWeServe.map((step: any, idx: number) => (
              <div key={idx} className="relative z-10 flex flex-col items-center text-center">
                <div className="w-24 h-24 rounded-full bg-[var(--bg-main)] border-[8px] border-[var(--bg-surface-elevated)] flex items-center justify-center text-[var(--accent-primary)] text-xl font-black mb-6 shadow-sm">
                  {idx + 1}
                </div>
      ...
      {catalogSec.show && (
        <section className="py-20 px-6 max-w-5xl mx-auto border-t border-[var(--border)]">
          <div className="flex flex-col items-center justify-center text-center mb-12">
            ...
            <h2 className="text-xl font-extrabold font-serif">{catalogSec.title}</h2>
          </div>
          ...
          {tenant.products && tenant.products.length > 0 ? (
            <div className="flex flex-col gap-12 max-w-4xl mx-auto">
              {Object.keys(groupedProducts).map((category) => (
                <div key={category}>
                  <h3 className="text-xl font-bold border-b-2 border-[var(--accent-primary)] pb-2 mb-6 inline-block pr-8">{category}</h3>
      ...
      {testimonialsSec.show && config.testimonials && config.testimonials.length > 0 && (
        <section className="py-20 px-6 max-w-6xl mx-auto border-t border-[var(--border)]">
          <div className="text-center mb-16">
            <h2 className="text-xl font-extrabold font-serif">{testimonialsSec.title}</h2>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
            {config.testimonials.map((t: any, idx: number) => (
              <div key={idx} className="bg-[var(--bg-surface-elevated)] p-5 rounded-[2rem] border border-[var(--border)] shadow-sm flex flex-col items-center text-center">
                <div className="w-20 h-20 rounded-full border-4 border-[var(--border)] overflow-hidden mb-6 bg-[var(--bg-main)]">
                  {t.avatarUrl ? (
                    ...
                    <div className="w-full h-full flex items-center justify-center text-xl text-[var(--text-secondary)] font-bold bg-[var(--bg-main)]">
                      {t.name?.charAt(0) || '?'}
                    </div>
                  )}
                </div>
      ...
      {gallerySec.show && config.gallery && config.gallery.length > 0 && (
        <section className="py-16 px-6 max-w-5xl mx-auto border-t border-[var(--border)]">
          <div className="text-center mb-10">
            <h2 className="text-xl font-extrabold font-serif">{gallerySec.title}</h2>
          </div>
        ...
        <section className="py-20 px-6 max-w-6xl mx-auto border-t border-[var(--border)]">
          <div className="flex flex-col md:flex-row bg-[var(--bg-surface-elevated)] rounded-[2rem] border border-[var(--border)] overflow-hidden shadow-sm">
            ...
            <div className="md:w-1/2 p-10 md:p-12 flex flex-col justify-center">
              ...
              <h2 className="text-xl font-extrabold font-serif mb-6">{contactSec.title}</h2>
              ...
              {waNumber && (
                <div className="mt-4 pt-6 border-t border-[var(--border)]">
                  ...
                  <a href={`https://wa.me/${waNumber.replace(/[^0-9]/g, '')}?text=Halo%20${tenant.name},%20saya%20ingin%20reservasi%20tempat:`} target="_blank" rel="noreferrer" className="inline-flex items-center justify-center gap-2 px-6 py-2.5 rounded-xl border border-[var(--border)] font-bold text-sm hover:border-green-500 hover:text-green-500 transition-all">
